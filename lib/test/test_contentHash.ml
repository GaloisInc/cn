...
let test_alpha_renaming _ =
  ...
  (* Build: sym1_a + sym1_b *)
  let it1 =
    IT.add_ (IT.sym_ (sym1_a, BT.Integer, loc), IT.sym_ (sym1_b, BT.Integer, loc)) loc
  in
  (* Build: sym2_a + sym2_b *)
  let it2 =
    IT.add_ (IT.sym_ (sym2_a, BT.Integer, loc), IT.sym_ (sym2_b, BT.Integer, loc)) loc
  in
...
let test_user_vars_preserved _ =
  ...
  (* Build: x + y *)
  let it1 = IT.add_ (IT.sym_ (x, BT.Integer, loc), IT.sym_ (y, BT.Integer, loc)) loc in
  (* Build: a + b *)
  let it2 = IT.add_ (IT.sym_ (a, BT.Integer, loc), IT.sym_ (b, BT.Integer, loc)) loc in
  let hash1 = CH.hash_index_term it1 in
  let hash2 = CH.hash_index_term it2 in
