              ...
              List.iter
                (fun (sym, (loc, args_and_body)) ->
                   ...
                   | Some record ->
                     ...
                     let pred_deps =
                       VerificationDb.get_predicate_dependencies db sym_str
                     in
                     let logfn_deps =
                     ...
                     in
                     let struct_deps =
                       VerificationDb.get_struct_dependencies db sym_str
                     in
                     let datatype_deps =
                     ...
                     let check_dep_staleness () =
                           ...
                           (fun callee ->
                              ...
                              | Some callee_rec ->
                                ...
                                (match Hashtbl.find_opt sym_name_map callee with
                                 | Some callee_sym ->
                                   (match
                                      Sym.Map.find_opt callee_sym global.fun_decls
                                    with
                                    | Some (_, ft_opt, _) ->
                              ...
                              | Some pred_rec ->
                                ...
                                (match Hashtbl.find_opt pred_name_map pred with
                                 | Some (_pred_sym, pred_def) ->
                                   let current_hash =
                                     ContentHash.hash_predicate pred_def
                                   in
                                   if
                                     String.compare
                       ...
                       let stale_logfns =
                         List.filter_map
                           (fun logfn ->
                              match
                                VerificationDb.get_logical_function_status db logfn
                              with
                              | Some logfn_rec ->
                       ...
                       let stale_structs =
                         List.filter_map
                           (fun struct_name ->
                              match
                                VerificationDb.get_struct_definition db struct_name
                              with
                              | Some struct_rec ->
  ...
  let only_flag =
    Arg.(
      ...
      & info
          [ "only" ]
          ~docv:"FUNCTION"
          ~doc:"Only show cache status for this function (for --cache-status)")
  in
  let wf_t =
    const well_formed
    $ Common.Flags.file
